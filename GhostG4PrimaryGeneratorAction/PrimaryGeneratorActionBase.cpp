...
Ghost::G4::PrimaryGeneratorActionBase::PrimaryGeneratorActionBase()
	: m_generator_name("UNKNOWNGENERATOR"),
		m_event_num(0),
...
void Ghost::G4::PrimaryGeneratorActionBase::GeneratePrimaries(G4Event* anEvent) {
	G4cout << "Generating primary particles from generator: " << m_generator_name
				 << " for event " << m_event_num << G4endl;

	ClearParticleVectors();
	FillParticleVectors();

	G4cout << "There are " << GetNParticlesToBeSimulated()
				 << " (" << GetNParticlesToBeSaved() << ")"
